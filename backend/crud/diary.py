...
class DiaryCrudManager:
    async def create(self, uid: str, newDiary: DiarySchema.DiaryCreate, db_session: AsyncSession):
        newDiary_dict = newDiary.model_dump()
        diary = DiaryModel(uid=uid, release_time=datetime.now().strftime("%Y-%m-%dT%H:%M:%S"),**newDiary_dict)
